...
7. `SECURITY.md`
8. `logrusbox/fleet` when the task has cross-component implications
...
- Vincent component intent and boundaries live in `docs/PRODUCT.md`.
...
- `logrusbox/fleet` owns the overall Fleet roadmap, cross-component integration issues, and Fleet governance; this repository owns Vincent component behavior only.
...
Vincent is public/reusable Fleet worker software. It contains installer/ISO tooling, first boot/runtime, worker status/diagnostics/network recovery, update/maintenance logic, Git/project connection, AI-provider adapters, tests, public-safe docs and releases.

CIC Station is Fleet's separate optional control plane. Worker-local packages/services/paths must use Vincent-local terminology; reserve `CIC Station` for the actual control-plane component/integration surface.
...
- Update requirements/ADRs when behavior/architecture changes; do not hide component changes only in code or roadmap prose.
